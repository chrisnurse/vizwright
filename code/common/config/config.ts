...
export default class Config {
  ...
  private static readonly SCRATCH_LOGS_DIR = "tests/scratch/scratch-logs";

  // Port configuration from environment variables
  private static readonly SERVER_PORT = parseInt(Deno.env.get("VIZWRIGHT_SERVER_PORT") ?? "3000");
  private static readonly UI_PORT = parseInt(Deno.env.get("VIZWRIGHT_UI_PORT") ?? "5173");
  ...
  /** Get the frontend UI port */
  static get UiPort(): number {
    return this.UI_PORT;
  }
  ...
  /** Get the scratch logs directory path, optionally with a sub-path */
  static ScratchLogsPath(subPath?: string): string {
    return this.buildPath(this.SCRATCH_LOGS_DIR, subPath);
  }
  ...
  static async EnsureTmpDirs(): Promise<void> {
    ...
    await Deno.mkdir(this.ScratchLogsPath(), { recursive: true });
  }
