# Page snapshot

```yaml
- generic [ref=e1]:
  - navigation [ref=e2]:
    - heading "🔐 Dashboard App" [level=1] [ref=e3]
    - generic [ref=e4]:
      - generic [ref=e5]: Regular User
      - button "Logout" [ref=e6] [cursor=pointer]
  - generic [ref=e7]:
    - generic [ref=e9]:
      - heading "Welcome back, Regular User!" [level=2] [ref=e10]
      - paragraph [ref=e11]: Comprehensive dashboard with forms, tables, modals, and interactive elements for testing.
    - generic [ref=e12]:
      - button "Overview" [ref=e13] [cursor=pointer]
      - button "Forms" [ref=e14] [cursor=pointer]
      - button "Data Table" [active] [ref=e15] [cursor=pointer]
      - button "Components" [ref=e16] [cursor=pointer]
      - button "Advanced" [ref=e17] [cursor=pointer]
    - generic [ref=e19]:
      - generic [ref=e20]:
        - heading "Tasks Management" [level=3] [ref=e21]
        - textbox "Search tasks..." [ref=e22]
      - table [ref=e24]:
        - rowgroup [ref=e25]:
          - row "ID ↕ Task ↕ Assignee ↕ Status ↕ Actions" [ref=e26]:
            - columnheader "ID ↕" [ref=e27] [cursor=pointer]
            - columnheader "Task ↕" [ref=e28] [cursor=pointer]
            - columnheader "Assignee ↕" [ref=e29] [cursor=pointer]
            - columnheader "Status ↕" [ref=e30] [cursor=pointer]
            - columnheader "Actions" [ref=e31] [cursor=pointer]
        - rowgroup
```